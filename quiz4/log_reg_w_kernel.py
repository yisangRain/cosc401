...
def logistic_regression_with_kernel(X, y, k, alpha, iterations):
    ...
    for epoch in range(iterations):
        for i in range(X.shape[0]):
            ...
            prediction = sigmoid(k(theta, x) + bias)

            #compute gradients
            gradient_theta = x * (z - prediction)

            gradient_bias = z - prediction

            theta = theta + alpha * gradient_theta
            bias = bias + alpha * gradient_bias

    def model(x, theta=theta, bias=bias, k=k):
        return sigmoid(np.dot(theta, x.T) + bias)
    return model
